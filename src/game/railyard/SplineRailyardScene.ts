...
export abstract class BaseSplineRailyardScene implements Scene {
  ...
  private async loadBoxcarImage(): Promise<void> {
    try {
      ...
      console.log(
        `✅ Boxcar image loaded successfully: ${boxcarImage.width}x${
          boxcarImage.height
        } (aspect ratio: ${(boxcarImage.width / boxcarImage.height).toFixed(
          2
        )}:1)`
      );
    } catch (error) {
      console.warn(
        "❌ Failed to load boxcar image, falling back to rectangle rendering:",
        error
      );
  ...
  private loadLevel(level: RailyardLevel): void {
    ...
    level.trainCars.forEach((car) => {
      console.log(
        `Loading car: ${car.id} with target locomotive: ${car.targetLocomotive}`
      );
    ...
    console.log(
      `Spline level loaded: ${level.tracks.length} tracks, ${level.trainCars.length} cars, ${level.locomotives.length} locomotives`
    );
  }
  ...
  public handleInput(input: InputState): void {
    ...
    if (mousePressed) {
      console.log(
        `Mouse pressed at: ${input.mouse.position.x}, ${input.mouse.position.y}`
      );

      // Check for any entity at the position
      const entity = this.entitySystem.getEntityAtPosition(
        input.mouse.position
      );
      if (entity && this.entitySystem.isDraggable(entity)) {
        console.log(
          `Starting spline drag for entity: ${entity.id} (${entity.type})`
        );
  ...
  private checkLevelCompletion(): void {
    const requiredConnections =
      this.gameState.level.objectives.requiredConnections;
    console.log(
      `Required connections for level completion:`,
      requiredConnections
    );

    const isComplete =
      this.entitySystem.checkLevelCompletion(requiredConnections);
  ...
  private renderSplineTracks(ctx: CanvasRenderingContext2D): void {
    ...
    tracks.forEach((track) => {
      ...
      if (track.spline.length >= 2) {
        ctx.strokeStyle = track.occupied
          ? COLORS.TRACK_OCCUPIED
          : COLORS.TRACK_UNOCCUPIED;
        ctx.lineWidth = 8;
        ctx.lineCap = "round";
        ctx.lineJoin = "round";
        ...
        for (let offset of [-6, 6]) {
          ...
          for (let i = 0; i <= samples; i++) {
            ...
            if (i < samples) {
              ...
              const nextPos = this.trackSystem.getPositionOnSpline(
                track.spline,
                nextT
              );
  ...
  private renderLocomotives(ctx: CanvasRenderingContext2D): void {
    ...
    locomotives.forEach((locomotive) => {
      ...
      ctx.strokeStyle = locomotive.isActive
        ? COLORS.CAR_BORDER_NORMAL
        : COLORS.CAR_BORDER_DRAGGING;
      ctx.lineWidth = 3;
      ctx.strokeRect(
        locomotive.position.x,
        locomotive.position.y,
      ...
      // Chimney
      ctx.fillRect(
        locomotive.position.x + 5,
        locomotive.position.y - 3,
        LOCOMOTIVE.CHIMNEY_WIDTH,
        LOCOMOTIVE.CHIMNEY_HEIGHT
      );
      // Front detail
      ctx.fillRect(
        locomotive.position.x + locomotive.size.x - 8,
        locomotive.position.y + LOCOMOTIVE.FRONT_DETAIL_MARGIN,
        LOCOMOTIVE.FRONT_DETAIL_WIDTH,
        locomotive.size.y - 10
      );
  ...
  private renderTrainCars(ctx: CanvasRenderingContext2D): void {
    ...
    cars.forEach((car) => {
      ...
      ctx.strokeStyle = car.isDragging
        ? COLORS.CAR_BORDER_DRAGGING
        : isLinked
        ? COLORS.COMPLETION_SUCCESS
        : COLORS.CAR_BORDER_NORMAL;
  ...
  private renderUI(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.textAlign = "left";
    ctx.fillText(
      `Level ${gameData.currentLevel}: ${this.gameState.level.name}`,
      20,
      40
    );
    ...
    ctx.fillText(
      "Drag train cars along spline tracks",
      20,
